...
#     -threads 1 \
#     -b:v:0 512k  -c:v:0 libx264 -filter:v:0 "scale=640:360:force_original_aspect_ratio=decrease,pad=640:360:-1:-1:color=black"  \
...
# Fifth iteration (MS4, Nginx now serves the videos)
ffmpeg -i $1 \
    -map 0:v:0 -map 0:v:0 -map 0:v:0 \
    -threads 1 \
    -b:v:0 512k  -c:v:0 libx264 -filter:v:0 "scale=640:360:force_original_aspect_ratio=decrease,pad=640:360:-1:-1:color=black"  \
    ...
    -b:v:2 1024k -c:v:2 libx264 -filter:v:2 "scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:-1:-1:color=black" \
    -use_timeline 1 -use_template 1 -window_size 5 -seg_duration 10 -media_seg_name "${2}_chunk_\$Bandwidth\$_\$Number\$.m4s" -init_seg_name "${2}_init-stream\$RepresentationID\$.\$ext\$" -adaptation_sets "id=0,streams=v" \
    -hls_playlist false -f dash /var/www/public/media/$2.mpd;

ffmpeg -i $1 -vf "thumbnail,scale=320:180:force_original_aspect_ratio=decrease,pad=320:180:(ow-iw)/2:(oh-ih)/2:color=black" -vframes 1 /var/www/public/media/$2.jpg